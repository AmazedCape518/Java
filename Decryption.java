...
public class Decryption {
	    ...
	    public static BigInteger RSA(boolean encrypt, BigInteger msg){
	    	BigInteger output;
	    	if(encrypt){
	    		output = modExp(msg, publicKey, n);
	    	}
	    	else{
	    		output = modExp(msg, privateKey, n);
	    	}
	    	return output;
	    }
		...
		public static void main(String[] args) {
			...
			p = findNextPrime(new BigInteger(pString),20);
			...
			BigInteger cipher = RSA(true, message);
			System.out.println(cipher);
			BigInteger plain = RSA(false, cipher);
			System.out.println(plain);
			
			
			//BigInteger cipherText = encryption(publicKey, new BigInteger(msg), n);
			//BigInteger plainText = decryption(privateKey, cipherText, n);
			scan.close();
		}
}
